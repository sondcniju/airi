...
// Matches standard <|TAG:...|> and also fallback text like `ACT:"emotion"...`
// frequently generated by local LLMs that omit boundary brackets.
// Bounded to 250 characters max to prevent catastrophic backtracking during streaming.
const ACT_MARKER_RE = /<\|[\w-]+:[\s\S]{0,250}?(?:\|>|>)|(?:^|\n)\s*\*?ACT\s*:.*(?=\n|$)/gi

function parseAssistantDisplayText(text: string): DisplaySegment[] {
  if (!text.includes('<|') && !text.includes('ACT')) {
...
const resolvedSlices = computed<ChatSlices[]>(() => {
  if (props.message.slices?.length) {
    return props.message.slices.reduce<ChatSlices[]>((acc, slice) => {
      ...
      acc.push({ ...slice, text: cleaned } satisfies ChatSlicesText)
      return acc
    }, [])
  }
...
function getMoodArchetype(text: string): string | null {
  ...
  if (!text.includes('<|ACT:') && !text.includes('['))
    return null

  // Pattern to find both ACT tags and Bracket tokens [mood]
  // Bounded to 250 characters max to prevent catastrophic backtracking.
  const matches = Array.from(text.matchAll(/<\|ACT:([\s\S]{0,250}?)\|>|\[([\w-]+)\]/gi))
